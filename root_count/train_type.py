...
from Models import smallVGGnet, densenet, densenet_lite, densenet_lite_type
import numpy as np
...
if __name__ == '__main__':
    ...
    if para.model_type == "densenet_lite_type":
        print('Initialising Densenet Lite ...')
        model = densenet_lite_type.DenseNet([3, 6, 12, 8])
    ...
    if not para.data_gen:
        # Training Set
        train_images, train_labels, img_names = dataset_utils.image_class_label_processor_2(para.train, para.img_rows, para.img_cols)
        train_images = train_images.astype(np.float32)
        train_images = train_images / 255.0
        train_labels = np_utils.to_categorical(train_labels, para.type_classes)

        # Validation Set
        val_images, val_labels, _ = dataset_utils.image_class_label_processor_2(para.val, para.img_rows, para.img_cols)
        val_images = val_images.astype(np.float32)
        val_images = val_images / 255.0
        val_labels = np_utils.to_categorical(val_labels, para.type_classes)
        ...
        # Training Set
        train_images, train_labels, _ = dataset_utils.image_class_label_processor_2(para.train, para.img_rows, para.img_cols)
