...
def SegNet(input_shape=(para.img_rows, para.img_cols, para.channels), classes=para.seg_classes, input_tensor=None, alpha=1.0):
    ...
    x = Conv2D(int(64 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    x = Activation("relu")(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)

    x = Conv2D(int(128 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    x = Activation("relu")(x)
    x = Conv2D(int(128 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    ...
    x = Conv2D(int(256 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    x = Activation("relu")(x)
    x = Conv2D(int(256 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    ...
    x = Conv2D(int(512 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
    x = Activation("relu")(x)
    x = Conv2D(int(512 * alpha), (3, 3), padding="same")(x)
    x = BatchNormalization()(x)
