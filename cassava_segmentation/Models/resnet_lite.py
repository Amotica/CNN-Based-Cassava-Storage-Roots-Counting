...
def _conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), dilation_rate=(1, 1), alpha=1):
    #   filter array
...
def build_resnet_blocks(input_tensor, kernel_size, filters, group=1, strides=(2, 2), _identities=2, alpha=1):
    if K.image_data_format() == 'channels_last':
        bn_axis = 3
    else:
        bn_axis = 1
...
def ResNet18(include_top=False, input_tensor=None, input_shape=(para.img_rows, para.img_cols, para.channels), alpha=1):
...
def ResNet34(include_top=False, input_tensor=None, input_shape=(para.img_rows, para.img_cols, para.channels), alpha=1):
