...
def load_synthetic_mask(synthetic_mask_dir, is_testing=True, cls=0):
    ...
    for msk in synthetic_mask_paths:
        ...
        msk = imresize(msk, img_res)
        cv2.imwrite("mask_png_1.png", msk)
        # If training => do random flip
        if not is_testing and np.random.random() < 0.5:
            msk = np.fliplr(msk)
...
def train_val_test_real_into_class(csv_file="cassava_real/count.csv"):
    with open(csv_file) as csvfile:
        ...
        for row in reader:
            ...
            test_path = "cassava/test/"
            testannot_path = "cassava/testannot/"
            testannot_path_rgb = "cassava/testannot_rgb/"

            test_file = test_path + "_0/" + row['filename']
            testannot_file = testannot_path + "_0/" + row['filename']
            testannot_file_rgb = testannot_path_rgb + "_0/" + row['filename']
            ...
            if os.path.isfile(test_file):
                ...
                testannot_folder_rgb = testannot_path_rgb + str(row['storage_roots']) + "/"
                if not os.path.exists(testannot_folder_rgb):
                    os.makedirs(testannot_folder_rgb, mode=0o777)
                # now save the file in this created folder
                testannot_image_rgb = cv2.imread(testannot_file_rgb)
                testannot_image_file_rgb = testannot_folder_rgb + row['filename']
                cv2.imwrite(testannot_image_file_rgb, testannot_image_rgb)

            # check is exist in test set. If exists then save in appropriate class in test set
            trainval_path = "cassava/trainval/"
            trainvalannot_path = "cassava/trainvalannot/"
            trainvalannot_path_rgb = "cassava/trainvalannot_rgb/"

            trainval_file = trainval_path + "_0/" + row['filename']
            trainvalannot_file = trainvalannot_path + "_0/" + row['filename']
            trainvalannot_file_rgb = trainvalannot_path_rgb + "_0/" + row['filename']
            ...
            if os.path.isfile(trainval_file):
                ...
                trainvalannot_folder_rgb = trainvalannot_path_rgb + str(row['storage_roots']) + "/"
                if not os.path.exists(trainvalannot_folder_rgb):
                    os.makedirs(trainvalannot_folder_rgb, mode=0o777)
                # now save the file in this created folder
                trainvalannot_image_rgb = cv2.imread(trainvalannot_file_rgb)
                trainvalannot_image_file_rgb = trainvalannot_folder_rgb + row['filename']
                cv2.imwrite(trainvalannot_image_file_rgb, trainvalannot_image_rgb)
...
def rename_images(image_dir, mask_dir, classes=para.num_classes):
    for cls in range(classes):

        image_paths = glob.glob(image_dir + str(cls) + "/" + "*.jpg") + glob.glob(
            image_dir + str(cls) + "/" + "*.png") + glob.glob(
            image_dir + str(cls) + "/" + "*.jpeg") + glob.glob(
            image_dir + str(cls) + "/" + "*.PNG") + glob.glob(
            image_dir + str(cls) + "/" + "*.JPG") + glob.glob(
            image_dir + str(cls) + "/" + "*.JPEG")
        image_paths.sort()
        mask_paths = glob.glob(mask_dir + str(cls) + "/" + "*.jpg") + glob.glob(
            mask_dir + str(cls) + "/" + "*.png") + glob.glob(
            mask_dir + str(cls) + "/" + "*.jpeg") + glob.glob(
            mask_dir + str(cls) + "/" + "*.PNG") + glob.glob(
            mask_dir + str(cls) + "/" + "*.JPG") + glob.glob(
        ...
        for image_path in image_paths:
            ...
            image_file =image_path[:-8] + ".png"
            print(image_file)
            cv2.imwrite(image_file, image)

        for mask_path in mask_paths:
            mask = cv2.imread(mask_path)
            mask_file = mask_path[:-9] + ".png"
            print(image_file)
...
if __name__ == '__main__':
    ...
    # check the generated images to see if the unique colours in the masks are good. code to make values unique in dataset_utils (bottom)
    '''image = cv2.imread("./cassava_gan/synthetic_mask/0/1_syn.png")
    unique_pixels = np.vstack({tuple(r) for r in image.reshape(-1, 3)})
    print(unique_pixels)

    image = cv2.imread("0_150_imgs_B_1.png")
    ...
    '''
    # get the validate images by evaluation with the cassava segmentation network before splitting into train val test.
    # use evaluate.py

    # validate the real images by evaluating with the cassava segmentation network and get predicted masks (trainval only)
    # again, use evaluate.py

    # train_validation_test_splits(image_dir="800/", classes=para.num_classes, train_ratio=0.8)

    # split the other dataset into the same classes as the previous....

    # rename images in trainval and trainvalannot
    #rename_images(image_dir, mask_dir, classes=para.num_classes)
