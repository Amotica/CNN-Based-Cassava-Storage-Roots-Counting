...
def rgb_pixels_to_binary(images_path="leaf/testannot/", target_path="leaf/testannot_new/"):
    ...
    for img_path in image_paths:
        ...
        image[np.where((image == [1, 1, 1]).all(axis=2))] = [0, 0, 0]
        image[np.where((image == [25, 51, 101]).all(axis=2))] = [1, 1, 1]
...
def image_depth_processor(image_dir, depth_dir, img_rows, img_cols):
    image_paths = glob.glob(image_dir + "0/" + "*.jpg") + glob.glob(image_dir + "0/" + "*.png") + glob.glob(
        image_dir + "0/" + "*.tiff")
    image_paths.sort()
    depth_paths = glob.glob(depth_dir + "0/" + "*.jpg") + glob.glob(depth_dir + "0/" + "*.png") + glob.glob(
        depth_dir + "0/" + "*.tiff")
    depth_paths.sort()

    img = []
    dpt = []
    for img_path, dpt_path in zip(image_paths, depth_paths):
        image = cv2.imread(img_path)
        image = cv2.resize(image, (img_cols, img_rows), interpolation=cv2.INTER_NEAREST)
        img.append(image)

        depth = cv2.imread(dpt_path)
        depth = cv2.resize(depth, (img_cols, img_rows), interpolation=cv2.INTER_NEAREST)
        depth = cv2.cvtColor(depth, cv2.COLOR_RGB2GRAY)
        dpt.append(depth)

    dpt = np.expand_dims(dpt, axis=3)
    return np.array(img), np.array(dpt)


def prepare_evaluation_images(image_dir, mask_dir, img_rows, img_cols, img_rows_out, img_cols_out, classes=para.num_classes):
    image_paths = glob.glob(image_dir + str(classes) + "/" + "*.jpg") + glob.glob(image_dir  + str(classes) + "/" + "*.png") + glob.glob(
        image_dir  + str(classes) + "/" + "*.jpeg") + glob.glob(
        image_dir  + str(classes) + "/" + "*.PNG") + glob.glob(
        image_dir  + str(classes) + "/" + "*.JPG") + glob.glob(
        image_dir + str(classes) + "/" + "*.JPEG")
    image_paths.sort()
    mask_paths = glob.glob(mask_dir  + str(classes) + "/" + "*.jpg") + glob.glob(mask_dir  + str(classes) + "/" + "*.png") + glob.glob(
        mask_dir + str(classes) + "/" + "*.jpeg") + glob.glob(
        mask_dir + str(classes) + "/" + "*.PNG") + glob.glob(
        mask_dir + str(classes) + "/" + "*.JPG") + glob.glob(
        mask_dir + str(classes) + "/" + "*.JPEG")
    mask_paths.sort()
...
if __name__ == '__main__':
    ...
    #rgb_pixels_to_binary(images_path="./cassava_fore_back_640x480/testannot_rgb/900_0/0/",
                        #target_path="./cassava_fore_back_640x480/testannot_new/900_0/0/")
    # 3. Get the class weights
    #class_weights("./cassava_fore_back/trainvalannot/0/")
    #class_weights("./cassava_multi_class/trainvalannot/0/")
